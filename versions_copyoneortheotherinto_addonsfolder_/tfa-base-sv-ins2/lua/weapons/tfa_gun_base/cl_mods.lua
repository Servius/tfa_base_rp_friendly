...
function SWEP:DrawWorldModel()
	...
	if IsValid(ply) and culldistancecvar and ply:GetPos():Distance(EyePos()) > culldistancecvar:GetFloat() and culldistancecvar:GetInt() ~= -1 then return end
...
function SWEP:UpdateBonePositions(vm)
	if self.ViewModelBoneMods then
		...
		for _,k in pairs(keys) do
			...
			while (cur ~= -1) do
				local pscale = (loopthrough[vm:GetBoneName(cur)] or defScaleTbl).scale
