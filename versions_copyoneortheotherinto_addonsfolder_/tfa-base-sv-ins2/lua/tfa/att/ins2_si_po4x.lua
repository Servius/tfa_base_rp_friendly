...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		local att, ts
		if wep:VMIV() then
			att = wep.OwnerViewModel:GetAttachment( wep.RTAttachment_PO4X or 0 )
		end
		...
		if wep.ScopeAngleTransforms_PO4X then
			ang:RotateAroundAxis(ang:Right(), wep.ScopeAngleTransforms_PO4X.p )
			ang:RotateAroundAxis(ang:Up(), wep.ScopeAngleTransforms_PO4X.y )
			ang:RotateAroundAxis(ang:Forward(), wep.ScopeAngleTransforms_PO4X.r )
		end
		cd.angles = ang
		...
		if ts then
			...
			if wep.ScopeOverlayTransforms_PO4X then
				scrpos.x = scrpos.x + wep.ScopeOverlayTransforms_PO4X[1]
				scrpos.y = scrpos.y + wep.ScopeOverlayTransforms_PO4X[2]
			end
			...
			if wep.ScopeOverlayTransformMultiplier_PO4X then
				scrpos.x = scrpos.x * wep.ScopeOverlayTransformMultiplier_PO4X
				scrpos.y = scrpos.y * wep.ScopeOverlayTransformMultiplier_PO4X
			end
			...
			local rtow, rtoh = 0, 0
			if wep.RTScopeOffset_PO4X then
				rtow = self.RTScopeOffset_PO4X[1] * rttw
				rtoh = self.RTScopeOffset_PO4X[2] * rtth
			end
			local rtw, rth = rttw * 1, rtth * 1
			if self.RTScopeScale_PO4X then
				rtw = rtw * self.RTScopeScale_PO4X[1]
				rth = rth * self.RTScopeScale_PO4X[2]
			end
			local distfac = math.pow( 1 - math.Clamp( ( att.Pos:Distance( wep.Owner:GetShootPos() ) - ( wep.ScopeDistanceMin_PO4X or 6 ) ) / ( wep.ScopeDistanceRange_PO4X or 12 ), 0, 1 ), 1 )
