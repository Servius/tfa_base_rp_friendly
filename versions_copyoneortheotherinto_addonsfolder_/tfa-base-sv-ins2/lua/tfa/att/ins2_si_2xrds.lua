...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		local att, ts
		if wep:VMIV() then
			att = wep.OwnerViewModel:GetAttachment( wep.RTAttachment_2XRDS or 0 )
		end
		...
		if wep.ScopeAngleTransforms_2XRDS then
			ang:RotateAroundAxis(ang:Right(), wep.ScopeAngleTransforms_2XRDS.p )
			ang:RotateAroundAxis(ang:Up(), wep.ScopeAngleTransforms_2XRDS.y )
			ang:RotateAroundAxis(ang:Forward(), wep.ScopeAngleTransforms_2XRDS.r )
		end
		cd.angles = ang
		...
		if ts then
			...
			if wep.ScopeOverlayTransforms_2XRDS then
				scrpos.x = scrpos.x + wep.ScopeOverlayTransforms_2XRDS[1]
				scrpos.y = scrpos.y + wep.ScopeOverlayTransforms_2XRDS[2]
			end
			...
			if wep.ScopeOverlayTransformMultiplier_2XRDS then
				scrpos.x = scrpos.x * wep.ScopeOverlayTransformMultiplier_2XRDS
				scrpos.y = scrpos.y * wep.ScopeOverlayTransformMultiplier_2XRDS
			end
			...
			local rtow, rtoh = 0, 0
			if wep.RTScopeOffset_2XRDS then
				rtow = self.RTScopeOffset_2XRDS[1] * rttw
				rtoh = self.RTScopeOffset_2XRDS[2] * rtth
			end
			local rtw, rth = rttw * 1, rtth * 1
			if self.RTScopeScale_2XRDS then
				rtw = rtw * self.RTScopeScale_2XRDS[1]
				rth = rth * self.RTScopeScale_2XRDS[2]
			end
			local distfac = math.pow( 1 - math.Clamp( ( att.Pos:Distance( wep.Owner:GetShootPos() ) - ( wep.ScopeDistanceMin_2XRDS or 3 ) ) / ( wep.ScopeDistanceRange_2XRDS or 10 ), 0, 1 ), 1 )
